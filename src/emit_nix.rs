...
/// `fetchgit`. `nixpkgs` is a Nix expression for the nixpkgs to use (e.g. a
/// `/nix/store` path the sync passes in).
fn fetch_git_lib(nixpkgs: &str) -> String {
    format!(
        "# Generated by guix-transfer. Fetcher for translated Guix git-fetch\n\
         ...
         \x20 inherit url rev hash name fetchSubmodules;\n\
         }}\n"
    )
}
...
pub fn emit_dir(
    ...
    if !git_sources.is_empty() {
        fs::write(out_dir.join("fetch-git.nix"), fetch_git_lib(nixpkgs))
...
mod tests {
    ...
    #[test]
    fn fetch_git_lib_uses_given_nixpkgs() {
        let lib = fetch_git_lib("/nix/store/abc-nixpkgs");
        assert!(lib.contains("import /nix/store/abc-nixpkgs"));
