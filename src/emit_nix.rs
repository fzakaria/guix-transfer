...
pub fn emit(out_path: &Path, translated: &[TranslatedDrv]) -> Result<(), String> {
    ...
    for (i, td) in translated.iter().enumerate() {
        ...
        if output_names.len() > 1
            || output_names.first().copied() != Some("out")
        {
...
pub fn emit_dir(out_dir: &Path, translated: &[TranslatedDrv]) -> Result<(), String> {
    ...
    for td in translated {
        let drv_filename = Path::new(&td.nix_drv_path)
            ...
            .unwrap()
            .replace(".drv", ".nix");
        output_to_file.insert(td.nix_drv_path.clone(), (drv_filename.clone(), "drvPath".to_string()));
        for (out_name, out_path) in &td.nix_outputs {
...
fn interpolate_multi(
    s: &str,
    output_to_file: &HashMap<String, (String, String)>,
) -> String {
    if !s.contains("/nix/store/") {
        return nix_str_literal(s);
    }
    ...
    for m in STORE_PATH_RE.find_iter(s) {
        ...
        if let Some((drv_filename, out_name)) = output_to_file.get(path) {
            ...
            replacements.push((m.start(), m.end(), interp));
        } else if path.contains("-source") || path.contains("-builder") || path.contains("-patch") || !path.ends_with(".drv") {
