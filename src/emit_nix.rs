...
/// Render a git-source `.nix`: an attrset with `out` bound to a `builtins.path`
/// (to give the checkout Guix's store name) wrapping a pinned `builtins.fetchGit`.
fn git_source_nix(gs: &GitSource) -> String {
    let submod = if gs.submodules {
        " submodules = true;"
    } else {
        ""
    };
    // `exportIgnore = false`: Guix's git-fetch is a plain checkout and does NOT
    // honor `.gitattributes export-ignore`; Nix's fetchGit does by default,
    // which strips files (and outright fails on some repos, e.g. brotli) and
    // diverges from Guix's hash. Disabling it makes the tree match.
    format!(
        "# Generated by guix-transfer (git source)\n{{ out = builtins.path {{ name = {name}; path = builtins.fetchGit {{ url = {url}; rev = {rev}; exportIgnore = false;{submod} }}; }}; }}\n",
...
pub fn emit_dir(
    ...
    // Git sources: each is its own `.nix` (fetchGit), referenced like a single-
    // output derivation via `(import <file>).out`, so the same interpolation and
    // srcs machinery handles them. Register them in output_to_file and write the
    // files.
    ...
    // Copy unique sources to sources/ (git sources are fetched at eval, not
    // copied — they live in their own .nix above).
    let mut copied_sources = std::collections::HashSet::new();
    for td in translated {
        for src_path in &td.drv.input_srcs {
            if copied_sources.contains(src_path) || git_sources.contains_key(src_path) {
                continue;
            }
    ...
    for td in translated {
        ...
        if !td.drv.input_srcs.is_empty() {
            nix.push_str("  srcs = [\n");
            for src in &td.drv.input_srcs {
                if let Some((filename, out_name)) = output_to_file.get(src) {
                    // A git source: reference its fetchGit `.nix` rather than a
                    // copied file.
                    nix.push_str(&format!("    (import ../store/{filename}).{out_name}\n"));
                } else {
...
pub fn verify_consistency(out_dir: &Path, translated: &[TranslatedDrv]) -> Result<(), String> {
    ...
    // so the check ignores them rather than crashing.
    let expr = format!(
...
mod tests {
    ...
    #[test]
    fn git_source_nix_renders_pinned_fetchgit() {
        let gs = GitSource {
            nix_path: "/nix/store/h-foo-checkout".into(),
            url: "https://github.com/x/y".into(),
            rev: "deadbeefdeadbeefdeadbeefdeadbeefdeadbeef".into(),
            name: "foo-0.1-checkout".into(),
            submodules: false,
        };
        let s = git_source_nix(&gs);
        assert!(s.contains("builtins.fetchGit"));
        assert!(s.contains("url = \"https://github.com/x/y\""));
        assert!(s.contains("rev = \"deadbeefdeadbeefdeadbeefdeadbeefdeadbeef\""));
        assert!(s.contains("name = \"foo-0.1-checkout\""));
        assert!(
            s.contains("exportIgnore = false;"),
            "must disable export-ignore to match Guix"
        );
        assert!(!s.contains("submodules"), "no submodules when false");

        let gs2 = GitSource {
            submodules: true,
            ..gs
        };
        assert!(git_source_nix(&gs2).contains("submodules = true;"));
    }
}
