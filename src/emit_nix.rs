...
/// The shared `fetch-git.nix` helper file: [`fetch_git_fn`] plus a header.
fn fetch_git_lib(nixpkgs_rev: &str) -> String {
    format!(
        "# Generated by guix-transfer.\n{}",
        fetch_git_fn(nixpkgs_rev)
    )
}

/// The bare git fetcher lambda over a pinned nixpkgs, shared verbatim with the
/// splicer's prefetch eval so both instantiate the identical derivation — the
/// drvPath guard in every consumer file compares the two.
///
/// nixpkgs is reached via `builtins.getFlake` with a pinned rev — the only form
/// that imports in pure flake evaluation (`import <store-path>` is forbidden
/// there, and `<nixpkgs>` needs NIX_PATH).
pub fn fetch_git_fn(nixpkgs_rev: &str) -> String {
    let flake = crate::splicer::nixpkgs_flake_ref(nixpkgs_rev);
    include_str!("fetch-git.nix").replace("\"@NIXPKGS_FLAKE@\"", &nix_str_literal(&flake))
}
...
/// Render a git-source `.nix`: a call into `fetch-git.nix`, the build-time FOD
/// that reproduces Guix's git checkout — read from Guix's build farms when they
/// still hold it, cloned otherwise. Either way the daemon fetches it lazily (and
/// cacheably) rather than during the sync.
fn git_source_nix(gs: &GitSource) -> String {
    format!(
        "# Generated by guix-transfer (git source)\n(import ../fetch-git.nix) {{ url = {url}; rev = {rev}; hash = {hash}; name = {name}; guixHash = {guix_hash}; fetchSubmodules = {sub}; }}\n",
        url = nix_str_literal(&gs.url),
        rev = nix_str_literal(&gs.rev),
        hash = nix_str_literal(&gs.hash_sri),
        name = nix_str_literal(&gs.name),
        guix_hash = nix_str_literal(&gs.guix_hash),
...
mod tests {
    ...
    fn git_source_nix_renders_fetchgit_call() {
        ...
        assert!(s.contains("guixHash = \"aryv06nlwjc8rk5smaaay73x1n8jx0qm\""));
    ...
    // Also asserts the getFlake is hoisted into a `let` outside the lambda,
    // for the same reason as the fetch_url_fn test above, and that the emitted
    // file is nothing but a header plus the shared fetcher — the splicer's
    // prefetch inlines that same text to record matching drvPaths.
    #[test]
    fn fetch_git_lib_uses_getflake_pinned_rev() {
        ...
        assert!(lib.contains(&fetch_git_fn("deadbeef")), "{lib}");
        let let_pos = lib.find("let\n").expect("hoisted let");
        let lambda_pos = lib.find("url,").expect("lambda head");
        assert!(let_pos < lambda_pos, "getFlake must be hoisted:\n{lib}");
    }

    /// Tests that the emitted fetcher reads Guix's build farms before it clones,
    /// by asserting both substituter hosts appear and that the clone fallback
    /// comes after them. Cloning first is what savannah's rate limiting turns
    /// into a hard failure.
    #[test]
    fn fetch_git_fn_prefers_guix_substitutes_over_cloning() {
        let f = fetch_git_fn("deadbeef");
        let bordeaux = f
            .find("bordeaux.guix.gnu.org")
            .expect("bordeaux substituter");
        let ci = f.find("ci.guix.gnu.org").expect("ci substituter");
        let clone = f
            .find("nix-prefetch-git --builder")
            .expect("clone fallback");
        assert!(bordeaux < ci, "bordeaux is tried first:\n{f}");
        assert!(ci < clone, "cloning is the last resort:\n{f}");
        assert!(f.contains("nix-store --restore"), "{f}");
        assert!(!f.contains("@NIXPKGS_FLAKE@"), "placeholder left:\n{f}");
    }

    /// Tests that the emitted file is valid Nix that instantiates: evaluates it
    /// against the pinned nixpkgs for one real source and asserts the resulting
    /// derivation's outPath is the content-addressed path that (name, hash)
    /// imply. That path is what consumers baked in, so this checks the property
    /// the whole fetcher rests on — changing how a checkout is fetched rebuilds
    ...
    fn fetch_git_lib_instantiates_the_content_addressed_path() {
        ...
        const CHECKOUT_GUIX_HASH: &str = "aryv06nlwjc8rk5smaaay73x1n8jx0qm";
        const EXPECTED_OUT: &str =
            "/nix/store/1982fyd5nbgiad50ggrak1884s4xyqhz-config-0.0.0-1.c8ddc84-checkout";

        let expr = format!(
            "(({lib}) {{ url = \"https://git.savannah.gnu.org/git/config.git/\"; rev = {rev}; hash = {hash}; name = {name}; guixHash = {guix_hash}; }}).outPath",
            lib = fetch_git_lib(crate::splicer::DEFAULT_NIXPKGS_REV),
            rev = nix_str_literal(CHECKOUT_REV),
            hash = nix_str_literal(CHECKOUT_HASH),
            name = nix_str_literal(CHECKOUT_NAME),
            guix_hash = nix_str_literal(CHECKOUT_GUIX_HASH),
        );
        let out = Command::new("nix")
            .args(["eval", "--impure", "--raw", "--expr", &expr])
            .output()
            .expect("running `nix eval` for the emitted fetcher");
        assert!(
            out.status.success(),
            "the emitted fetch-git.nix does not evaluate:\n{}",
