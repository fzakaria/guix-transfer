...
//! function of the package-set fixpoint (`{ pkgs }: builtins.derivation { … }`)
//! and references its peers through `pkgs."<store-basename>"` rather than
//! `import`-ing them directly. Routing every edge through that indirection lets
//! the consuming flake apply overlays (e.g. disabling a flaky `check` phase) and
//! have the change propagate through the whole transitive graph. Source inputs
//! (`-builder`/`-source`/`-patch`) stay as `builtins.path` references.
...
pub fn emit_dir(
    ...
    for td in translated {
        ...
        nix.push_str("# Generated by guix-transfer\n");
        // Each store file is a function of the package-set fixpoint so that
        // dependencies resolve through `pkgs.<key>` (an overridable indirection)
        // instead of a hard-coded `import ../store/<file>.nix`. This makes the
        // whole graph overlayable from the consuming flake.
        nix.push_str("{ pkgs }:\n");
...
fn interpolate_multi(s: &str, output_to_file: &HashMap<String, (String, String)>) -> String {
    ...
    for m in STORE_PATH_RE.find_iter(s) {
        ...
        if let Some((drv_filename, out_name)) = output_to_file.get(path) {
            // Reference peer derivations through the package-set fixpoint keyed
            // by the store-file basename (e.g. `pkgs."<hash>-m4-boot0-1.4.19".out`),
            // rather than `import`-ing the file directly. This keeps every edge
            // routed through an overridable indirection so the graph is overlayable.
            let key = drv_filename.strip_suffix(".nix").unwrap_or(drv_filename);
            let interp = format!("${{pkgs.{}.{out_name}}}", nix_str_literal(key));
            replacements.push((m.start(), m.end(), interp));
...
mod tests {
    ...
    #[test]
    fn interpolate_multi_refs_pkgs_fixpoint() {
    ...
    #[test]
    fn interpolate_multi_refs_pkgs_drvpath() {
        let mut m = HashMap::new();
        m.insert(
        ...
        assert_eq!(
            ...
            "\"${pkgs.\"bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb-foo\".drvPath}\""
        );
    }
